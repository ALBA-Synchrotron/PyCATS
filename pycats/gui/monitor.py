...
class CS8State(QtCore.QObject):
    ...
    def set(self, param, value):
        ...
        if values is not None:
            ...
            if previous_value != value:
                self.emit(QtCore.SIGNAL('paramChanged(QString)'),QtCore.QString(param))
        if param == 'message':
    ...
    def getMessageHelp(self, message):
        if not message_help.has_key(message):
            return None
...
class CS8_Panel(QtGui.QFrame):
    ...
    def __init__(self, parent=None, cs8=None):
        ...
        QtCore.QObject.connect(self.cs8, QtCore.SIGNAL('paramChanged(QString)'),
                               self.paramChanged)
    ...
    def build_contents(self):
        ...
        for p in di_params:
            if p != '.':
                ...
                tooltip = 'Unchecked: %s\nChecked: %s' % (param_help[0], param_help[1])
        ...
        for p in do_params:
            if p != '.':
                ...
                if label_text.startswith('PRO') and not label_text.startswith('PROCESS'):
                    label.font().setBold(True)
                widget = QtGui.QCheckBox()
                widget.setFixedSize(15,15)
                checked = (value == '1')
...
class WidgetController:
    ...
    def valueChanged(self, value):
        ...
        if self.param == 'LIFE_BIT_COMING_FROM_PLC':
            return
        self.widget.setStyleSheet('%s { background-color: yellow }' % widget_class)
        if self.reset_stylesheet_timer is not None:
            QtCore.QObject.disconnect(self.reset_stylesheet_timer,QtCore.SIGNAL("timeout()"),self.resetStyleSheet)
        self.reset_stylesheet_timer = QtCore.QTimer(self.parent)
        self.reset_stylesheet_timer.setSingleShot(True)
        QtCore.QObject.connect(self.reset_stylesheet_timer,QtCore.SIGNAL("timeout()"),self.resetStyleSheet)
...
class MonitorCS8(QtGui.QApplication):
    def __init__(self, *args):
        ...
        QtCore.QObject.connect(update_timer, QtCore.SIGNAL('timeout()'), self.update_status)
